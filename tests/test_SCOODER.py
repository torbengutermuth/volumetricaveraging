...
from VolumetricAveraging import VolumetricAveraging
import os
import rdkit
import pandas as pd
...
class TestScooder(TestCase):
    def setUp(self):
        ...
        self.args_nosc = {
            "modus": "nosc",
            "dockingposes": os.path.join(
                self.start, "testfiles", "05_moe_prepared.sdf"
            ),
            "gridsize": 0.5,
            "features": "normal",
        }
    ...
    def test_init(self):
        herbert = VolumetricAveraging(self.args_nosc)
    ...
    def test_transform_mols(self):
        ...
        to_compare = pd.read_csv(
            os.path.join(self.start, "testfiles", "features_raw_moeprep.csv"),
            index_col=0,
            header=0,
            names=["A", "B", "C", "D", "E"],
    ...
    def test_extract_information(self):
        ...
        to_compare = pd.read_csv(
            os.path.join(self.start, "testfiles", "features_processed_moreprep.csv"),
            header=0,
            index_col=0,
            dtype=datatypes,
        )
    ...
    def test_color_dataframe(self):
        ...
        to_compare = pd.read_csv(
            filepath_or_buffer=os.path.join(
                self.start, "testfiles", "color_df_moeprep.csv"
            ),
            header=0,
            index_col=0,
        )
    ...
    def test_write_density(self):
        herbert = VolumetricAveraging(self.args_query)
    ...
    def test_read_configs(self):
        shutil.copytree(
            os.path.join(self.start, "testfiles", "adrb2_fred"), "adrb2_fred"
        )
        shutil.copytree(
            os.path.join(self.start, "testfiles", "adrb2_hybrid"), "adrb2_hybrid"
        )
        herbert = VolumetricAveraging(self.args_analysis)
    ...
    def test_run_analysis2(self):
        ...
        herbert = VolumetricAveraging(self.args_analysis2)
        self.assertEqual(True, herbert.run())
        self.assertEqual(80, len(os.listdir("grids_pdbs")))
        self.assertEqual(8, len(os.listdir("grids_densities")))
    ...
    def test_run_analysis3(self):
        ...
        herbert = VolumetricAveraging(self.args_analysis3)
        self.assertEqual(True, herbert.run())
        self.assertEqual(80, len(os.listdir("grids_pdbs")))
    ...
    def test_run_analysis4(self):
        ...
        herbert = VolumetricAveraging(self.args_analysis4)
        self.assertEqual(True, herbert.run())
        self.assertEqual(80, len(os.listdir("grids_pdbs")))
